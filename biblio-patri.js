...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const addLabelToggleControl = (mapInstance, mapType) => {
        const LabelControl = L.Control.extend({
            onAdd: function() {
                const container = L.DomUtil.create('div', 'leaflet-control-label-toggle');
                const btn = L.DomUtil.create('button', '', container);
                btn.innerHTML = 'Étiquettes';
                L.DomEvent.on(btn, 'click', L.DomEvent.stopPropagation)
                    .on(btn, 'click', L.DomEvent.preventDefault)
                    .on(btn, 'click', () => { if (window.toggleLabels) { window.toggleLabels(mapType); } });
                return container;
            }
        });
        new LabelControl({ position: 'topleft' }).addTo(mapInstance);
    };
    ...
    const initializeMap = (params) => {
        ...
        addLabelToggleControl(map, 'analysis');
...
const initializeSelectionMap = (coords) => {
    ...
    const initializeObservationMap = () => {
        ...
        addLabelToggleControl(obsMap, 'observations');
