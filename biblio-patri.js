...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const displayResults = (occurrences, patrimonialMap) => {
        ...
        sortedSpecies.forEach((species, index) => {
            ...
            row.innerHTML = `<td><span class="legend-color" style="background-color:${color};"></span><i>${species.name}</i></td><td>${species.label}</td><td>${species.occurrences.length}</td><td><a href="https://inpn.mnhn.fr/espece/recherche?q=${encodeURIComponent(species.name)}" target="_blank" rel="noopener noreferrer">Fiche</a></td>`;
        ...
        table.innerHTML = `<thead><tr><th>Nom scientifique</th><th>Statut</th><th>Occurrences (${SEARCH_RADIUS_KM}km)</th><th>Lien INPN</th></tr></thead>`;
